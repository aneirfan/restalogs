...
const buatTemplateDetailResto = (restaurant) => `
  ...
  <img class="rest__poster lazyload" data-src="${CONFIG.BASE_IMAGE_URL_M + restaurant.pictureId}" alt="${restaurant.name}" />
    ...
    <div class="detail-review">
                ${restaurant.customerReviews.map((review) => `<div class="detail-review-item">
                            <div class="review-header">
                                <p class="review-name"><img class="lazyload" data-src="./images/people.png" alt="reviewer ${review.name}">&nbsp;${review.name} <span tabindex="0"> (${review.date}) </span></p>
  ...
  <div class="resto-item">
    ...
    <div class="resto-item__header">
      <a href="${`/#/detail/${resto.id}`}">
        <img class="resto-item__header__poster lazyload" data-src="${CONFIG.BASE_IMAGE_URL_S + resto.pictureId}" alt="restoran ${resto.name}">
      </a>
      <div class="kota">${resto.city}</div>
      <div class="resto-item__header__rating">
            <p>Rating:<span class="resto-item__header__rating__score">${resto.rating}</span></p>
      </div>
    </div>
    <div class="resto-item__content">
        <h3>
          <a href="${`/#/detail/${resto.id}`}" class="resto_name">${resto.name}</a>
        </h3>
